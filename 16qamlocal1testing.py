...
for j in range(1,6):
	...
	start = time.time()
	prg.ml_decoder(queue, (2,N) , (2,1), r_buf, input_received_buf, P_buf, Q_buf, P_inv_buf, constellation_buf, np.int32(N), np.int32(M), np.float32(HfGf_sqr)).wait()
	print "size is : ", N*4,"Running time GPU:",time.time()-start 
