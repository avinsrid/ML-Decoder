...
for j in (1,11):
	...
	print "N =",N*4,"     CPU running time",time.time()-start
	...
	start = time.time()
	prg.ml_decoder(queue, (M,M*N) , (M,M), r_buf, input_received_buf, P_buf, Q_buf, P_inv_buf, constellation_buf, np.int32(N), np.int32(M)).wait()
	print "N= ",N*4,"     GPU running time",time.time()-start
