            ...
            /* Check if Ms < temp, if TRUE, then store Ms in temp and store decoded_sbar and decoded_cbar with their respective values */
  			...
  			if (lx == 0 && ly == 0)
  			{
  				for (int i = 0; i < 256; i++) 
  				{
  					if (Ms[i] < temp)
  					{
  						decoded_sbar[0][0] = S_bar[0][i];
  						decoded_sbar[1][0] = S_bar[1][i];
  						decoded_cbar[0][0] = C_bar[0][i];
  						decoded_cbar[1][0] = C_bar[1][i];
  						temp = Ms[i];
  					}
            	}
...
prg.ml_decoder(queue, (M,M*N) , (M,M), r_buf, input_received_buf, P_buf, Q_buf, P_inv_buf, constellation_buf, np.int32(N), np.int32(M))
