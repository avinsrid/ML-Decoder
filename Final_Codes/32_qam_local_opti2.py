...
__kernel void ml_decoder(__global cfloat_t* decoded, __global cfloat_t* r, __global cfloat_t* MatP, __global cfloat_t* MatQ, __global cfloat_t* MatInvP, __global cfloat_t* QAMconstell, const int N,const int sizeQAM) {
        ...
        float superMs[4];
        cfloat_t superC_bar[2][4];
        cfloat_t superS_bar[2][4];
        cfloat_t Cs[2][1];
        ...
        for ( int d = 0; d < 2; d++)
        {
         for (int e = 0; e < 2; e++)
         {
           ...
           superMs[2*d+e] = 100000000.0;
           S_bar[0][16*lx + ly] = QAMconstell[lx + d*16];
           S_bar[1][16*lx + ly] = QAMconstell[ly + e*16]; 
           ...
           /* r - Qs */
           /* get_group_id(1) refers to the workgroup here, which directly refers to received symbols indices */
		   ...
		   for ( int k = 0 ; k < sizeQAM ; k++)
            ...
            /* Calculate Ms */
            /* Note that tempMs[][] and Ms[] are in local memory */

            tempMs[0][16*lx + ly] = cfloat_add(r[get_group_id(1) + 0], cfloat_add(-Pc[0][0], -Qs[0][0]));
            tempMs[1][16*lx + ly] = cfloat_add(r[get_group_id(1) + N], cfloat_add(-Pc[1][0], -Qs[1][0]));
            barrier(CLK_LOCAL_MEM_FENCE);
            ...
            /* Check if Ms < temp, if TRUE, then store Ms in temp and store decoded_sbar and decoded_cbar with their respective values */
  			...
  			if (lx == 0 && ly == 0)
  			{
  				for (int i = 0; i < 256; i++) 
  				{
  					if (Ms[i] < superMs[2*d + e])
  					{
  						superS_bar[0][2*d + e] = S_bar[0][i];
  						superS_bar[1][2*d + e] = S_bar[1][i];
  						superC_bar[0][2*d + e] = C_bar[0][i];
  						superC_bar[1][2*d + e] = C_bar[1][i];
  						superMs[2*d + e] = Ms[i];
  					}
            	}
        	}
        ...
        if ( lx == 0 && ly == 0)
        {
        	int Ms_temp = 1000000000.0;
        	for (int temp = 0; temp < 4; temp++)
        	{
        		if (Ms_temp > superMs[temp])
        		{
        			Ms_temp = superMs[temp];
        			
        			decoded_cbar[0][0] = superC_bar[0][temp];
        			decoded_cbar[1][0] = superC_bar[1][temp];
        			decoded_sbar[0][0] = superS_bar[0][temp];
        			decoded_sbar[1][0] = superS_bar[1][temp];
...
constellation = np.array([-3 + 5j, -5 - 1j,  3 + 3j, -1 - 3j, -5 + 3j, 3 - 1j, -1 + 1j, -3 - 5j,  1 + 5j, \
						  -1 - 1j, -5 + 1j,  3 - 3j, -1 + 3j, -5 - 3j, 3 + 1j,  1 - 5j, -1 + 5j, -3 - 1j, \
						   5 + 3j,  1 - 3j, -3 + 3j,  5 - 1j, 1 + 1j, -1 - 5j,  3 + 5j,  1 - 1j, -3 + 1j, \
						   5 - 3j,  1 + 3j, -3 - 3j,  5 + 1j, 3 - 5j]).astype(np.complex64)
...
print "Total number of bits transmitted = ", 5 * 4 * N
print "Bit error rate = ", bit_err/(5*4*N)"""
opencl_recvsymbols = np.zeros_like(result_decode)
...
start = time.time()
var = prg.ml_decoder(queue, (M/2,(M/2)*N) , (M/2,M/2), r_buf, input_received_buf, P_buf, Q_buf, P_inv_buf, constellation_buf, np.int32(N), np.int32(M))
var.wait()
time_opencl = time.time() - start
